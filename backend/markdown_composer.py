...
## 테이블 작성 규칙 (필수!)
- 모든 테이블 바로 위에 반드시 제목 1줄을 붙일 것. 형식: 이모지 + 설명 (예: "📊 종목별 평가 요약", "🌐 현재 시장 신호 점검", "⚖️ 현재 vs 목표 비중 비교", "⚠️ 위험 종목 신호 현황")
- 제목은 bold/헤딩 없이 이모지+텍스트 한 줄로만 작성 (예: 📊 종목별 평가 요약)
- 테이블 제목이 없으면 사용자가 표의 의미를 파악할 수 없음 — 반드시 붙일 것
...
핵심 요약 1~2문장 → 주요 지표 테이블 → 해석 → 주의 신호 나열 → 액션 제안
