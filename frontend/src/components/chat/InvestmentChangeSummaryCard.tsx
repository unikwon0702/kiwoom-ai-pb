...
export type InvestmentChangeSummaryItem = {
  asset_name: string;
  tag: string;
  desc: string;
  sub_desc?: string;
...
export type InvestmentChangeSummaryData = {
  ai_summary?: string;
  items: InvestmentChangeSummaryItem[];
};

type Props = {
  data: InvestmentChangeSummaryData;
  onItemClick: (assetName: string) => void;
};

export function InvestmentChangeSummaryCard({ data, onItemClick }: Props) {
  return (
    <div className="rounded-2xl bg-card border border-border/70 overflow-hidden">
      ...
      <div className="space-y-2 px-4 pt-2 pb-3">
        {data.items.length === 0 ? (
          <p className="text-[13px] text-muted-foreground py-3 text-center">변동 내역이 없어요</p>
        ) : (
          data.items.map((it, i) => (
            <button
              key={i}
              onClick={() => onItemClick(it.asset_name)}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="flex-1 text-[14.5px] font-bold text-foreground truncate">{it.asset_name}</span>
                {it.time && <span className="text-[12px] text-foreground/60 shrink-0">{it.time}</span>}
                <ChevronRight className="size-4 text-muted-foreground shrink-0" />
              </div>
              <p className="mt-1.5 pl-[42px] text-[13px] text-foreground/85 leading-[1.5]">{it.desc}</p>
              {it.sub_desc && <p className="mt-1 pl-[42px] text-[12.5px] text-muted-foreground leading-[1.5]">{it.sub_desc}</p>}
            </button>
          ))
        )}
      </div>
