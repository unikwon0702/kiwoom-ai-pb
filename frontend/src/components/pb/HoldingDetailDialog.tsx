...
function BuyGauge({ buyCount, sellCount, pollLabel }: { buyCount: number; sellCount: number; pollLabel?: string }) {
  ...
  // Buy on the LEFT, Sell on the RIGHT. Boundary slides based on buy ratio.
            ...
            {chart && (
              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-[12px] font-semibold text-muted-foreground tracking-wide">
                    <span>📉</span>
                    <span>{chart.title ?? "코스피 대비 수익률 (최근 1개월)"}</span>
                  </div>
                  {chart.gap && (
                ...
                {chart.caption && (
                  <p className="text-[12.5px] text-muted-foreground leading-relaxed pt-1">{chart.caption}</p>
                )}
              </section>
            )}
                  ...
                  {history.map((h, i) => {
                    ...
                    return (
                      <li key={i} className="relative">
                        <span
                          className={`absolute -left-[11px] top-3 size-2 rounded-full ring-2 ring-background ${
                          ...
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 text-[12px] font-bold px-2 py-0.5 rounded-full ${
                                isUp
                                  ? "bg-[color:var(--pos-soft)] text-[color:var(--pos)]"
                                  : "bg-[color:var(--neg-soft)] text-[color:var(--neg)]"
                              }`}
                            >
            ...
            <section>
              <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
                ...
                <div className="px-4 pt-4 pb-3">
                  <div className="flex items-center gap-1.5 text-[12px] font-semibold text-muted-foreground tracking-wide">
                    <span>🏆</span>
                    <span>고수들은 어제까지 이 종목을 이렇게 거래했어요</span>
                  </div>
                ...
                {!hideMasterDetails && (
                  <>
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`size-4 text-muted-foreground transition-transform ${showMasters ? "rotate-180" : ""}`}
                      />
                    </button>
                        ...
                        {mastersList.map((m, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center justify-between rounded-xl border border-border/60 px-3.5 py-3">
                              <div className="flex items-center gap-2.5">
                                <span className={`size-7 rounded-full flex items-center justify-center text-[13px] ${isBuy ? "bg-[color:var(--pos-soft)]" : "bg-[color:var(--neg-soft)]"}`}>{m.emoji}</span>
                                <div>
                              ...
                              <span className={`flex items-center gap-1 text-[12px] font-bold ${isBuy ? "text-[color:var(--pos)]" : "text-[color:var(--neg)]"}`}>
