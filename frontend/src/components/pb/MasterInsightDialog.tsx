...
function buildDailyPicks(investors: any[], masters: Master[]): DailyPick[] {
  return investors.map((inv, i) => {
    const buysRaw = inv.daily_buys_json ? JSON.parse(inv.daily_buys_json) : [];
    const sellsRaw = inv.daily_sells_json ? JSON.parse(inv.daily_sells_json) : [];
    return {
      master: masters[i] ?? masters[0],
      date: inv.daily_pick_date ? `${inv.daily_pick_date} 기준` : "최근 기준",
      buys: buysRaw.map((b: any, idx: number) => ({
        ticker: b.asset_name ?? b.ticker ?? `종목${idx+1}`,
        sector: b.sector ?? "",
        weight: b.weight ?? Math.max(10, 40 - idx * 8),
      })),
      sells: sellsRaw.map((s: any, idx: number) => ({
        ticker: s.asset_name ?? s.ticker ?? `종목${idx+1}`,
        sector: s.sector ?? "",
        weight: s.weight ?? Math.max(10, 40 - idx * 8),
      })),
    };
  });
}
...
function buildPortfolios(investors: any[], masters: Master[]): Portfolio[] {
  return investors.map((inv, i) => {
    ...
    if (sectorAllocation.length === 0) {
      sectorAllocation.push({ label: sectorRaw.top_sector ?? "기타", value: 100, color: SECTOR_COLORS[0] });
    }

    const domesticTop = domesticRaw.map((d: any) => ({
      label: d.asset_name ?? d.label ?? "",
      value: d.return_pct ?? d.value ?? 0,
    }));
    ...
    return {
      master: masters[i] ?? masters[0],
      totalKRW: inv.total_asset_krw ?? "—",
      avgHoldings: inv.avg_holdings ? `${inv.avg_holdings}개` : "—",
      turnover: "—",
      domesticRatio: 70,
      sectorAllocation,
      domesticTop,
...
function mockQuote(ticker: string, weight: number, tone: "buy" | "sell") {
  ...
  const buyNotes = ["거래량 급증, 단기 모멘텀 강화", "외국인 순매수 유입세 지속", "실적 기대감, 업종 내 상대강도 상승", "기관 매집 흔적, 추세 전환 신호", "배당·실적 안정성 부각"];
  const sellNotes = ["단기 과열, 차익실현 압력", "수급 약화, 추세 둔화 감지", "실적 모멘텀 둔화, 비중 축소", "기술적 저항선 부담", "리스크 헤지 차원 정리"];
...
function BubbleMap({ picks, tone }: { picks: Pick[]; tone: "buy" | "sell" }) {
  ...
  if (picks.length === 0) {
    return (
      <div className="relative h-[200px] rounded-2xl bg-muted/30 flex items-center justify-center">
        <p className="text-[13px] text-muted-foreground">{isBuy ? "매수" : "매도"} 데이터 없음</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative h-[200px] rounded-2xl bg-muted/30 overflow-hidden">
      {picks.map((p, i) => {
        ...
        const positions = [
          { left: "50%", top: "50%" }, { left: "32%", top: "32%" },
          { left: "68%", top: "34%" }, { left: "34%", top: "70%" },
          { left: "68%", top: "70%" }, { left: "50%", top: "20%" },
        ];
            ...
            <PopoverTrigger asChild>
              <button
                ...
                style={{
                  ...
                  fontSize: Math.max(8.5, Math.min(11, size / 9)), lineHeight: 1.1,
                }}
              >
            ...
            <PopoverContent side="top" align="center" className="w-60 p-3">
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: softColor, color: baseColor }}>
                    {isBuy ? "매수" : "매도"}
                  </span>
                </div>
      ...
      <div className="absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full pointer-events-none" style={{ backgroundColor: softColor, color: baseColor }}>
        {isBuy ? "매수" : "매도"}
      </div>
    </div>
  );
      ...
      <PopoverTrigger asChild>
        <div className="h-[120px] w-full cursor-pointer">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} dataKey="value" innerRadius="55%" outerRadius="95%" paddingAngle={1} stroke="none" onClick={(_, idx) => setSelected(idx)}>
                {data.map((_, i) => (
                  <Cell key={i} fill={colors[i % colors.length]} style={{ cursor: "pointer", outline: "none" }} opacity={selected != null && selected !== i ? 0.45 : 1} />
                ))}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
      ...
      {sel && (
        <PopoverContent side="top" align="center" className="w-52 p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{side}</span>
          </div>
    ...
    <div className="rounded-2xl border border-border/60 bg-card p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="size-8 shrink-0 rounded-full flex items-center justify-center text-[14px]" style={{ backgroundColor: `color-mix(in oklab, ${d.master.accent} 14%, white)` }}>
            {d.master.emoji}
          </span>
        ...
        <div className="relative flex shrink-0 rounded-full bg-muted p-0.5 text-[11px] font-bold">
          ...
          <button onClick={() => setMode("buy")} className="relative z-10 px-3 py-1 flex items-center gap-1" style={{ color: isBuy ? "var(--pos)" : "hsl(var(--muted-foreground))" }}>
            <TrendingUp className="size-3" />매수
          </button>
          <button onClick={() => setMode("sell")} className="relative z-10 px-3 py-1 flex items-center gap-1" style={{ color: !isBuy ? "var(--neg)" : "hsl(var(--muted-foreground))" }}>
            <TrendingDown className="size-3" />매도
          </button>
        </div>
      </div>
...
export function MasterInsightDialog({ open, onOpenChange }: Props) {
  ...
  const dailyPicks = buildDailyPicks(investors, masters);
  const portfolios = buildPortfolios(investors, masters);
        ...
        <DrawerPrimitive.Content className="fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-[24px] border bg-background max-w-[480px] mx-auto focus:outline-none">
          ...
          <div className="px-6 pt-4 pb-3 space-y-7 max-h-[78vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button onClick={() => onOpenChange(false)} aria-label="닫기" className="size-8 -mr-1 shrink-0 rounded-full flex items-center justify-center text-muted-foreground hover:bg-muted/60">
                <X className="size-4" />
              </button>
            </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <section className="space-y-3">
                  ...
                  <Carousel items={dailyPicks} renderItem={(d) => <DailyPickCard pick={d} />} />
                </section>
