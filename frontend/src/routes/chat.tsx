...
import {
  initSession, detectCustomerSwitch, getCurrentSessionId, startNewSession,
  getSessionList, loadSessionMessages, saveSessionMessages, addSession,
  clearAllSessions
...
/* ===== Constants ===== */
// const HISTORY_KEY = "aipb_chat_questions"; // replaced by chatSession.ts
...
// loadHistory() replaced by chatSession.ts initSession/getSessionList
...
function ChatPage() {
  ...
  useEffect(() => {
    ...
    console.log(`[AI_PB_DEBUG] Customer changed: ${customer.id} (${customer.segmentCode})`);
  }, [customer.id]);
  ...
  const sendQuestion = async (text: string) => {
    ...
    setLoading(true);
    // History is auto-saved by the useEffect above (multi-session)
    try {
      ...
      <div className="max-w-[480px] mx-auto min-h-screen flex flex-col bg-[#F5F6FA]">
        ...
        <HistoryPanel open={menuOpen} onClose={() => setMenuOpen(false)} history={history}
          onSelect={(q, idx) => {
            ...
            const selected = list[idx !== undefined ? idx : list.findIndex(s => s.title === q)];
            if (selected) {
              window.__AIPB_SID = selected.id;
              setMessages(loadSessionMessages(selected.id) as Msg[]);
            } else {
              sendQuestion(q);
            }
          }}
...
/* ===== Follow-Up Questions ===== */
function FollowUpQuestions({ questions, onSelect }: { questions: string[]; onSelect: (q: string, idx?: number) => void }) {
  return (
